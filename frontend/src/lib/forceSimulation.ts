import type { Community, Entity, GraphData } from './graphData'
...
export class ForceSimulation3D {
  private simulation: d3.Simulation<Node3D, Link3D>
  ...
  constructor(config: ForceConfig = defaultForceConfig) {
    ...
    this.simulation = d3
      ...
      .force(
        'link',
        d3
          .forceLink<Node3D, d3.SimulationLinkDatum<Node3D>>()
  ...
  private preprocessData(graphData: GraphData): void {
    ...
    this.nodes = graphData.entities.map((entity, index) => {
      ...
      const maxAbstraction = Math.max(
        ...graphData.entities.map((e) => e.degree + e.frequency * 0.5),
      )
      ...
      const normalizedAbstraction =
        maxAbstraction > minAbstraction
          ? (abstractionScore - minAbstraction) / (maxAbstraction - minAbstraction)
          : 0.5
  ...
  private setupForces(): void {
    ...
    this.simulation
      .force('charge', d3.forceManyBody<Node3D>().strength(nodeChargeStrength))
      .force(
        'link',
        d3
          .forceLink<Node3D, d3.SimulationLinkDatum<Node3D>>()
      ...
      .force(
        'collision',
        d3.forceCollide<Node3D>().radius((d: Node3D) => {
    ...
    this.communities.forEach((community, communityIndex) => {
      ...
      if (communityNodes.length > 0) {
        ...
        const avgAbstraction =
          communityNodes.reduce((sum, node) => {
            return (
              sum + ((node as unknown as { abstractionLevel?: number }).abstractionLevel || 0.5)
            )
  ...
  private addSphericalConstraint(): void {
    ...
    this.simulation.force('spherical', () => {
      this.nodes.forEach((node) => {
        ...
        const maxRadius = this.config.spread3D
        const idealRadius =
          minRadius + (1 - (nodeWithAbstraction.abstractionLevel || 0.5)) * (maxRadius - minRadius)
  ...
  private updateIndividualForces(_oldConfig: ForceConfig, changes: Partial<ForceConfig>): void {
    ...
    if (
      changes.communityStrength !== undefined ||
  ...
  private precomputeCommunityData(): void {
    ...
    this.communities.forEach((community) => {
      ...
      const communityNodes = this.nodes.filter((node) => community.entity_ids.includes(node.id))
      ...
      if (community.parent !== null && community.parent !== undefined) {
        const parentId =
          typeof community.parent === 'string'
            ? Number.parseInt(community.parent)
            : community.parent
        const parentCommunity = this.communities.find(
          (c) => String(c.human_readable_id) === String(parentId),
        )
        if (parentCommunity) {
      ...
      this.communities.forEach((otherCommunity) => {
        if (otherCommunity.parent !== null && otherCommunity.parent !== undefined) {
          const parentId =
            typeof otherCommunity.parent === 'string'
              ? Number.parseInt(otherCommunity.parent)
              : otherCommunity.parent
          if (String(parentId) === String(community.human_readable_id)) {
      ...
      if (community.children && Array.isArray(community.children)) {
        community.children.forEach((childHumanId: string) => {
          const childId = Number.parseInt(childHumanId)
          const childCommunity = this.communities.find(
            (c) => String(c.human_readable_id) === String(childId),
          )

...
export class GraphDataLoader {
  ...
  async loadGraphData(): Promise<GraphData> {
    try {
      const [entitiesData, relationshipsData, communitiesData, reportsData] = await Promise.all([
      ...
      const entities = this.parseJsonToEntities(entitiesData as Record<string, unknown>[])
      const relationships = this.parseJsonToRelationships(
        relationshipsData as Record<string, unknown>[],
      )
      const communities = this.parseJsonToCommunities(communitiesData as Record<string, unknown>[])
      const communityReports = this.parseJsonToCommunityReports(
  ...
  private parseJsonToRelationships(data: Array<Record<string, unknown>>): Relationship[] {
    return data.map((item) => ({
      ...
      text_unit_ids: this.ensureStringArray(item.text_unit_ids),
      // JSON graphs carry no resolution flag; only live data marks unresolved edges
      unresolved: false,
    }))
  }
  ...
  private parseJsonToCommunities(data: Array<Record<string, unknown>>): Community[] {
    return data.map((item) => ({
      ...
      parent:
        item.parent !== undefined && item.parent !== -1 && item.parent !== '-1'
          ? String(item.parent as string | number)
          : undefined,
